...
func beginShow(eventId: int ):
	...
	Global.eventReward = eventDict[eventId][1]
	self.show()
	entranceAnimation()
...
func updateBaseStats(info): 
	var aux= "[" + str(info*Global.baseEnemyTemplatesStats[0]) + "," + str(info*Global.baseEnemyTemplatesStats[1]) + "," + str(info*Global.baseEnemyTemplatesStats[2]) + "," + str(info*Global.baseEnemyTemplatesStats[3]) + "," + str(info*Global.baseEnemyTemplatesStats[4]) +"]"
	get_node("BaseStatsContainer/Stats").text = aux
