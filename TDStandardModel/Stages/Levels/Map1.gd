...
func get_enemy_instance(e):
	...
	idNumber +=1
	var key= str(Global.currLevel)+ "-" + str(Global.currentRound)
	enemy.setup(Global.enemyStatTable[key])
	return enemy		
...
func _on_body_entered_goal(body):
	...
	Global.health -= round(body.damage*0.75)
	updateScore()
...
func updateScore():
	...
	var aux = Global.currentRound
	if(aux==0):
		aux=1
	var key= str(Global.currLevel) + "-" + str(aux)
	
	get_node("TowerSelection").updateBaseStats(Global.enemyStatTable[key])
...
func new_enemy_group_spawn():
	...
	nEnemies = 1 + randi_range(0,Global.currentRound+round(Global.intensityValue/2))
