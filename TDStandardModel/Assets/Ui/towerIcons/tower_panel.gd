...
func _on_gui_input(event):
	...
	elif get_child_count()>0 and event is InputEventMouseMotion and event.button_mask == 1 and blocked != 1:
		#"Dragging Tower" 
		if not get_children():
			print("Null positioning detected for hover event")
			return 
		get_child(1).global_position = event.global_position + Vector2(0,-20)
		if canPlace() and canBuyAux():
...
func canBuyAux() ->bool:
	return Global.currency >= currentCost
...
func canPlace() -> bool:
	...
	#print(tile)
	if(get_children() and get_child(1) != null):
